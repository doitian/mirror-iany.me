...
{{< image-card src="ConceptOS.jpg" caption="Concept OS" >}}

其中 conceptMAIL 是收到各种需求（也就是关卡）的地方。

{{< image-card src="conceptMAIL.jpg" caption="conceptMAIL" >}}

通过邮件可以进入到 CAD Pad 也就是游戏关卡的主界面。

{{< image-card src="CADPad.jpg" caption="CAD Pad" >}}

右边可以选择各种芯片，左边主要区域是一块电路板，可以把芯片放上去，然后通过电路连接引脚。下方是具体的需求说明以及验证用的测试用例。同时提供了断点，单步执行等调试工具。

{{< image-card src="Verification.jpg" caption="验证用例" >}}
...
{{< image-card src="Rank.jpg" caption="排名直方图" >}}

另一个挑战就是数字引脚的同步了，需要精心设计输入输出芯片的代码时序保存不会因为 IO 阻塞而无法执行命令。

{{< image-card src="IOBlocking.jpg" kg-width="fit" caption="IO 阻塞导致失败" >}}
