...
{{< callout type="file" icon="fas fa-file" title="min-width.css" fold="" >}}
...
The snippet does not work in horizontal splits where panes stack vertically. See the chapter [JavaScript Helper](#javascript-helper) below how to work around it.
...
{{< image-card src="enable-obsidian-min-width-css-snippet.png" caption="Enable CSS Snippet in Obsidian" >}}
...
{{< gallery-card "add-obsidian-min-width-macro.png|Add a Macro" "run-min-width-script-on-plugin-load.png|Run on plugin load" "|Run JavaScript Code on Start With QuickAdd" >}}

That's it. Restart Obsidian and the code should be effective. See how it works in the screen recording below.

{{< vimeo-card id="752964835" caption="Set the Minimum Width of the Active Pane in Obsidian" >}}
